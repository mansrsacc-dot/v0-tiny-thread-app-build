...
// The embroidery zone (hoodie chest patch / cap front) spans only THIS fraction of the
// preview/mockup width — it is a sub-region of the garment photo, NOT the whole canvas. So a design
// of `mm` renders at (mm / zoneWidth) of the zone, i.e. (mm / zoneWidth) × fraction of the canvas.
// TUNE THESE to match where the print area sits in each photo: raise if designs look too small,
// lower if too big. Per-product so hoodie and cap tune INDEPENDENTLY.
// VISUAL-ONLY: these scale the on-screen/email mockup render of designs. They do NOT touch pxToMm,
// so the "~XXmm" label, designer-email mm, and cart mm are unchanged for every product.
//   hoodie: 0.33 → 0.4125 (+25%) so chest designs render 25% bigger.
//   cap:    0.4125 → 0.515625 (a further +25%) — the cap front is small, so designs still looked
//           too small after the shared hoodie bump; this grows the cap mockup without touching mm.
const HOODIE_ZONE_PREVIEW_WIDTH_FRACTION = 0.4125;
const CAP_ZONE_PREVIEW_WIDTH_FRACTION = 0.515625;

// VISUAL-ONLY on-screen scale per garment size + product (used by BOTH the live preview and the
// email mockup). currentSizePx maps to canvas units; multiply by this so the design shows at its
// real proportion of the zone. Narrower zone (smaller garment) → same mm looks slightly bigger.
export const designRenderScale = (garmentSize: string, product: Product = "hoodie"): number =>
  (product === "cap" ? CAP_ZONE_PREVIEW_WIDTH_FRACTION : HOODIE_ZONE_PREVIEW_WIDTH_FRACTION)
    * (ZONE_REF_MM / (EMBROIDERY_ZONES[garmentSize as keyof typeof EMBROIDERY_ZONES]?.width ?? ZONE_REF_MM));
