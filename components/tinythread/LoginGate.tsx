...
// Shown when there's no session. "Log in" starts the Customer Account API OAuth flow:
//   /api/auth/login → account.tinythread.shop login → /api/auth/callback (sets the session cookie)
//   → back to the app, logged in. Works with new customer accounts (no server-side {{ customer }}).
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="bg-[#1e1b18] border border-white/10 rounded-2xl p-7">
          <p className="text-white/55 text-sm text-center mb-6 leading-relaxed">
            {lv
              ? "Pieslēdzies, lai izveidotu un saglabātu savus dizainus."
              : "Log in to create and save your designs."}
          </p>
          ...
          <p className="text-white/30 text-xs text-center mt-4 leading-relaxed">
            {lv
              ? "Tiksi novirzīts uz drošu Shopify pieslēgšanos un atpakaļ uz studiju."
              : "You'll be taken to secure Shopify login and back to the studio."}
          </p>
        </div>
